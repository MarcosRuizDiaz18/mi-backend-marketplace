...
export default function Login({ onNavigateRegister, onLoginSuccess }) {
  ...
  const validate = () => {
    const e = {};
    if (!form.email)    e.email    = 'Ingresá tu correo electrónico';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setAlert({ type: 'error', msg: data.message || 'Credenciales incorrectas. Revisá tu email y contraseña.' });
        return;
      }

      // Guardamos el token si el backend lo devuelve
      if (data.token) localStorage.setItem('equipya_token', data.token);
      if (data.usuario) localStorage.setItem('equipya_user', JSON.stringify(data.usuario));

      setAlert({ type: 'success', msg: `¡Bienvenido de vuelta${data.usuario?.nombre ? ', ' + data.usuario.nombre : ''}!` });
    ...
    <div className="auth-layout">
      ...
      <aside className="auth-brand">
        ...
        <div className="brand-tagline-block">
          <h2 className="brand-tagline">
            El equipamiento<br />que necesitás,<br /><span>cuando lo necesitás.</span>
          </h2>
          <p className="brand-description">
            Conectamos a quienes tienen equipo disponible con quienes lo necesitan. Rápido, seguro y sin complicaciones.
          </p>
        </div>

        <div className="brand-features">
          <div className="brand-feature"><span className="brand-feature-dot" />Publicá y alquilá equipos fácilmente</div>
          <div className="brand-feature"><span className="brand-feature-dot" />Transacciones seguras entre usuarios</div>
          <div className="brand-feature"><span className="brand-feature-dot" />Disponible las 24 horas, los 7 días</div>
        </div>
      </aside>
      ...
      <main className="auth-form-panel">
        <div className="auth-form-box">
          ...
          <p className="auth-subheading">Ingresá tus datos para acceder a tu cuenta</p>
