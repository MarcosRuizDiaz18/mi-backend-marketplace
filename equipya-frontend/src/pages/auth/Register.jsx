...
/* ── Field fuera del componente principal para evitar re-mount en cada tecla ── */
function Field({ id, label, icon, type = 'text', placeholder, autoComplete, value, onChange, error, showToggle, show, onToggle }) {
  return (
    <div className="form-field">
      <label className="form-label">{label}</label>
      <div className="input-wrap">
        <input
          className={`form-input ${showToggle ? 'has-toggle' : ''} ${error ? 'has-error' : ''}`}
      ...
      {error && <span className="field-error"><IconAlert />{error}</span>}
    </div>
  );
}
        ...
        <div className="auth-form-box">
          ...
          <form className="auth-form" onSubmit={handleSubmit} noValidate>
            ...
            <div className="field-row">
              <Field id="nombre" label="Nombre" icon={<IconUser />} placeholder="Juan"
                value={form.nombre} onChange={handleChange('nombre')} error={errors.nombre}
                autoComplete="given-name" />
              <Field id="apellido" label="Apellido" icon={<IconUser />} placeholder="García"
                value={form.apellido} onChange={handleChange('apellido')} error={errors.apellido}
                autoComplete="family-name" />
            </div>

            <Field id="email" label="Correo electrónico" icon={<IconMail />} type="email"
              placeholder="tu@email.com" value={form.email} onChange={handleChange('email')}
              error={errors.email} autoComplete="email" />

            <Field id="telefono" label="Teléfono (opcional)" icon={<IconPhone />} type="tel"
              placeholder="+54 11 0000-0000" value={form.telefono} onChange={handleChange('telefono')}
              autoComplete="tel" />

            <Field id="password" label="Contraseña" icon={<IconLock />}
              placeholder="Mínimo 6 caracteres" value={form.password} onChange={handleChange('password')}
            ...
            <Field id="confirmPassword" label="Confirmar contraseña" icon={<IconLock />}
              placeholder="Repetí tu contraseña" value={form.confirmPassword} onChange={handleChange('confirmPassword')}
